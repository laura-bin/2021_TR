/** *************************************************************************************
 * Labo B : Lecteurs - Ecrivains avec priorites
 * ============================================
 * 
 ...
 * arg[2]: readers priority
 * arg[3]: count of writer threads
 * arg[4]: writers priority
...
int main (int argc, char *argv[]) {
    ...
    // test the args
    if (argc != 6) {
        fprintf(stderr, "usage: %s readers readers_priority writers writers_priority max_value\n", argv[0]);
        return 1;
    }
    ...
    // initialize readers priority
    read_thread_params.priority = atoi(argv[2]);
    if (read_thread_params.priority < 1 || read_thread_params.priority > 99) {
        exit_code = 1;
        fprintf(stderr, "invalid readers priority\n");
        goto end;
    }
    ...
    // initialize writers priority (test already implemented by the nice function)
    write_thread_params.priority = atoi(argv[4]);
    if (write_thread_params.priority < 1 || write_thread_params.priority > 99) {
        exit_code = 1;
        fprintf(stderr, "invalid writers priority\n");
        goto end;
    }
    ...
    // open the log file
    if (open_stat_file(read_thread_params.readers_count, read_thread_params.priority,
            write_thread_params.writers_count, write_thread_params.priority, max)) {
